...
def send_file(server_ip, server_port):
    ...
    """
    logging.basicConfig(
    filename=f"server{server_port-6554}.log",
    filemode='w',
    level=logging.INFO,
    ...
    with open(FILE_PATH, 'rb') as file:
        ...
        dup_3_bool=False
        is_dup_acks=False 
        # for the calculation of the rtt and rto
        rtt_map={}
        rto=1.0
        srtt=1.0
        rttvar=1.0
        ...
        while not end_of_file or len(unacked_packets) > 0:
            ...
            if dup_3_bool:
                fast_recovery_fn(server_socket, client_address, unacked_packets,rtt_map)
            elif not is_dup_acks:
                seq_num=window_base
                past_time = time.time()
                # check if it is retransmission or not
                if retransmit:
                    for number in range(0,cwnd):
            ...
            if rto>60.0:
                rto=60.0
            # print('window size is ',cwnd)
            # print("rto is ",rto)
            # while((time.time()-past_time<rto) and (not received)):
            try:
                if (past_time+rto)-time.time()<0:
                ...
                if ack_seq_num==-1:
                    ...
                    if congestion_avoidance:
                        ca_count+=1
                        logging.info(f"in congestion avoidance and ca count {ca_count}")
                        logging.getLogger().handlers[0].flush() 
                        if ca_count==cwnd:
                            ca_count-=cwnd
                            cwnd=cwnd+1
                    ...
                    if congestion_avoidance:
                        logging.info(" after changing in congestion avoidance")
                        logging.getLogger().handlers[0].flush() 

                    if slow_start:
                        logging.info(" after changing in slow_start")
                        logging.getLogger().handlers[0].flush() 

                    if fast_recovery:
                        logging.info("after changing in fast_recovery")
                        logging.getLogger().handlers[0].flush() 
                    ...
                    if fast_recovery:
                        cwnd+=1
                        logging.info(f"in fast recovery increased cwnd by 1 and cwnd is {cwnd}")
                        logging.getLogger().handlers[0].flush()
                    else:
                        duplicate_ack_map[ack_seq_num]+=1
                        # is_dup_acks=True
                        # if duplicate_ack_map[ack_seq_num]==3:
                        #     dup_3_bool=True
                        #     duplicate_ack_map[ack_seq_num]=0
                        #     # print(f"Received 3 duplicate ACK for packet {ack_seq_num}")
                        
                        if duplicate_ack_map[ack_seq_num]==3 and slow_start:
                            logging.info(f"Received 3 duplicate ACK for packet {ack_seq_num} in slow start")
                            logging.getLogger().handlers[0].flush()
                        ...
                        elif duplicate_ack_map[ack_seq_num]==3 and congestion_avoidance:
                            logging.info(f"Received 3 duplicate ACK for packet {ack_seq_num} in congestion avoidance")
                            logging.getLogger().handlers[0].flush()
                            ssthresh=max(2,cwnd//2)
                            cwnd=ssthresh+3
                            congestion_avoidance=False
                ...
                '''
                    CAUTION !!!!!!!!
                    must think about the duplicate_ack_map[ack_seq_num]=0
                '''
                if slow_start:
                    logging.info(f"in slow start")
                    logging.getLogger().handlers[0].flush()
                    ssthresh=max(2,cwnd//2)
                    cwnd=1
                    
                if congestion_avoidance:
                    logging.info(f"in congestion avoidance")
                    logging.getLogger().handlers[0].flush()
                    ssthresh=max(2,cwnd//2)
                    cwnd=1
                    congestion_avoidance=False
                    slow_start=True
                if fast_recovery:
                    logging.info(f"in fast recovery")
                    logging.getLogger().handlers[0].flush()
                    ssthresh=max(2,cwnd//2)
                    cwnd=1
                    fast_recovery=False
                ...
                duplicate_ack_map[window_base]=0                   
                # retransmit_unacked_packets(server_socket, client_address, unacked_packets)
                retransmit=True

                logging.info(f'retransmit is {retransmit}')
                logging.getLogger().handlers[0].flush()

            logging.info(f'timeout is {rto} and cwnd is {cwnd} and ssthresh is {ssthresh}')
            logging.getLogger().handlers[0].flush()
...
def fast_recovery_fn(server_socket, client_address, unacked_packets,rtt_map):
    """
    ...
    logging.info(f"Fast retransmitted packet {earliest_seq_num}")
    logging.getLogger().handlers[0].flush()
...
def get_seq_no_from_ack_pkt(ack_packet):
    ...
    """
    # print("ack packet is ",ack_packet.decode())
