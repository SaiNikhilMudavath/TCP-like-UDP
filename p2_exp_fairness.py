...
def run():
    ...
    for DELAY in delay_list:
        for i in range(0, NUM_ITERATIONS):
            ...
            for file in ['1received_file.txt','2received_file.txt']:
            ...
            pref_c1 = "1"
            pref_c2 = "2"
            s1_cmd = f"python3 p2_server.py {SERVER_IP1} {SERVER_PORT1} > s1_output.log 2>&1 &"
            s2_cmd = f"python3 p2_server.py {SERVER_IP2} {SERVER_PORT2} > s2_output.log 2>&1 &"
            c1_cmd = f"python3 p2_client.py {SERVER_IP1} {SERVER_PORT1} --pref_outfile {pref_c1} > c1_output.log 2>&1  &"
            c2_cmd = f"python3 p2_client.py {SERVER_IP2} {SERVER_PORT2} --pref_outfile {pref_c2} > c2_output.log 2>&1  &"

            # s1_cmd = f"python3 p2_server.py {SERVER_IP1} {SERVER_PORT1}  &"
            # s2_cmd = f"python3 p2_server.py {SERVER_IP2} {SERVER_PORT2}  &"
            # c1_cmd = f"python3 p2_client.py {SERVER_IP1} {SERVER_PORT1} --pref_outfile {pref_c1} >c1_error.log 2>&1  &"
            # c2_cmd = f"python3 p2_client.py {SERVER_IP2} {SERVER_PORT2} --pref_outfile {pref_c2} >c2_error.log 2>&1  &"
