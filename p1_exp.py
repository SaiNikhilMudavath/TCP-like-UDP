...
with open('client.log','w') as file:
    pass
...
def suppress_output(func):
    """Suppress output of a function temporarily."""
...
def run(expname):
    ...
    f_out.write("loss,delay,fast_recovery,md5_hash,ttc,throughput\n")
        ...
        for DELAY in delay_list:
            for FAST_RECOVERY in [1,0]:
                for i in range(0, NUM_ITERATIONS):
                    ...
                    with open('client.log','a') as fileclog:
                        fileclog.write(f'{LOSS}% packet loss and fast recovery {FAST_RECOVERY} and iteration {i}\n')
                        fileclog.close()
                    with open('server.log','w') as fileslog:
                        fileslog.write(f'{LOSS}% packet loss and fast recovery {FAST_RECOVERY} and iteration {i}\n')
                        fileslog.close()
                    ...
                    h1.cmd(f"python3 p2_server.py {SERVER_IP} {SERVER_PORT} {FAST_RECOVERY} > s_output.log 2>&1 &")
                    result = h2.cmd(f"python3 p2_client.py {SERVER_IP} {SERVER_PORT} > c_output.log 2>&1")
                    end_time = time.time()
                    ttc = end_time-start_time
                    md5_hash = compute_md5('received_file.txt')
                    # shutil.copy('received_file.txt',f'received_file_{20*LOSS+5*FAST_RECOVERY+i}.txt')
                    # shutil.copy('received_file.txt',f'received_file_new.txt')
                    # write the result to a file 
                    f_out.write(f"{LOSS},{DELAY},{FAST_RECOVERY},{md5_hash},{ttc},{file_size/(ttc*1024*1024)}\n")
                    f_out.flush()
                            

                    # Stop the network
                    net.stop()
                    # suppress_output(cleanup)
